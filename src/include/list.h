...
#define LIST_DEFINE(type, typename)                                         \
    ...
    int typename##_init(typename **list);                                   \
    ...
    int typename##_contains(typename *list, typename##_predicate predicate, \
                            void *context, type **result);                  \
    int typename##_free(typename *list);

#define LIST_DECLARE(type, typename)                                          \
    int typename##_init(typename **list) {                                    \
        *list = calloc(1, sizeof(typename));                                  \
        if (*list == NULL) {                                                  \
            return -1;                                                        \
        }                                                                     \
        return 0;                                                             \
    ...
    int typename##_add(typename *list, type *value) {                         \
        if (list->capacity == 0) {                                            \
            ...
            list->capacity *= 2;                                              \
            list->data =                                                      \
                (type *) realloc(list->data, sizeof(type) * list->capacity);  \
        }                                                                     \
                                                                              \
        if (list->data == NULL) {                                             \
            return -1;                                                        \
...
#define foreach(elem, list)                               \
    for (int __##elem##__i = 0;                           \
         __##elem##__i < (list)->count                    \
             ? ((elem) = &(list)->data[__##elem##__i], 1) \
             : 0;                                         \
         ++__##elem##__i)
